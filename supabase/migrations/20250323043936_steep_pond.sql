/*
  # Add RLS policies and lessons data

  1. Security
    - Enable RLS on lessons table if not already enabled
    - Add policies if they don't exist:
      - Public read access to all lessons
      - Admin-only write access
  
  2. Data
    - Insert 14 chapters of Japanese learning content
*/
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'lessons' 
    AND policyname = 'Anyone can view lessons'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'lessons' 
    AND policyname = 'Only admins can modify lessons'
  ) THEN
